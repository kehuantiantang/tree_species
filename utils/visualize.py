...
def show_mask(images, masklist, Mean=(2, 2, 2), Std=(0.5, 0.5, 0.5)):
    ...
    for i in range(len(masklist)):
        mask = masklist[i].total_data.cpu()
        # for b in range(mask.size(0)):
        #     mask[b] = (mask[b] - mask[b].min())/(mask[b].max() - mask[b].min())
        mask_size = mask.size(2)
        # print('Max %f Min %f' % (mask.max(), mask.min()))
...
def weight2tensorboard(model, writer, epoch):
    for name, param in model.named_parameters():
        if '.weight' in name:
            writer.add_histogram('weight/%s'%name, param, epoch)
            if param.grad is not None:
                writer.add_histogram('grad/%s'%name, param.grad, epoch)
        elif '.bias' in name:
            writer.add_histogram('bias/%s'%name, param, epoch)
        else:
            pass
...
if __name__ == '__main__':
    ...
    model = resnet18()
    for name, param in model.named_parameters():
        print(name, param.size(), param.grad)
